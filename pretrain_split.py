...
class GPT2Trainer:
    def __init__(self, args, debug_mode=False):
        ...
        self.do_tokenize = args.do_tokenize  # 选择是否从零开始构建数据集
    ...
    def tokenize_and_save(self):
        ...
        all_len = len(content_list)
        token_cnt = 0
        for i in range(self.split_num):
            sub_content_list = content_list[all_len // self.split_num * i: all_len // self.split_num * (i + 1)]
            if i == self.split_num - 1:
                sub_content_list.extend(content_list[all_len // self.split_num * (i + 1):])  # 把尾部例子添加到最后一个piece
            all_ids = []
            for content in sub_content_list:
                ...
                all_ids.extend(ids)
            token_cnt += len(all_ids)
            with open(self.tokenized_data_path + 'tokenized_train_{}.txt'.format(i), 'w') as f:
                for id in all_ids:
                    f.write(str(id) + ' ')
        self.print_and_log("总文章数 = %d" % all_len)
        self.print_and_log("总字数 = %d" % token_cnt)
    ...
    def train(self):
        ...
        epoch_steps = int(full_len / self.stride / self.batch_size / self.gradient_accumulation)
        total_steps = int(full_len / self.stride * self.epochs / self.batch_size / self.gradient_accumulation)
        ...
        for epoch in range(self.epochs):
            ...
            optimizer.zero_grad()
            split_indices = np.linspace(0, self.split_num - 1, self.split_num, dtype=np.int32)
            random.shuffle(split_indices)
            for split_index in split_indices:
                with open(self.tokenized_data_path + 'tokenized_train_{}.txt'.format(split_index), 'r') as f:
                    line = f.read().strip()
                all_ids = line.split()
                ...
                for i in range(len(samples) // self.batch_size):  # drop last
                    ...
                    batch_labels = torch.tensor(batch, dtype=torch.long).to(self.device)
                    batch_inputs = torch.tensor(batch, dtype=torch.long).to(self.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--do_tokenize', action='store_true', help='是否先做tokenize')
    ...
    parser.add_argument('--split_num', default=10, type=int, required=False, help='将训练语料分成多少份')
