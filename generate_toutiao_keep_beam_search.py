    ...
    def generate_sequence(self, context_ids, keyword_ids, length, num_samples=1, temperature=1, top_k=0, top_p=0.0, is_xlnet=False, window_size=512):
        ...
        with torch.no_grad():
            for _ in range(length):
                for generated, sum_prob in beam:
                    inputs = {'input_ids': generated[:, -(window_size-1):], "keyword_ids": keyword_ids}
                    outputs = self.model(**inputs)
                    next_token_logits = outputs[0][0, -1, :] / temperature
                    filtered_logits = self.top_k_top_p_filtering(next_token_logits, top_k=top_k, top_p=top_p)
                    next_tokens = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=beam_size)
                    for token in next_tokens:
                        prob = filtered_logits[token]
                        buffer.append([torch.cat((generated, token.unsqueeze(0).unsqueeze(0)), dim=1), sum_prob + prob])
                buffer = sorted(buffer, key=lambda x: x[1], reverse=True)
                beam = buffer[:beam_size * 2]
                buffer = []
        generated = beam[0][0]
        return generated
