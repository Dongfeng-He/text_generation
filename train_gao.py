        ...
        if len(content) > self.min_length and len(keywords) > 0:
            ...
            while len(passage_tokens) > self.passage_min_length - 1:
                ...
                for keyword in keywords:
                    if keyword not in current_str: continue
        ...
        with open(self.raw_data_path, "r") as f:
            for i, line in enumerate(f):
                ...
                if len(keyword_ids_list) != 0 and len(passage_ids_list) != 0:
                    # TODO: 重要，如果关键词没在该片段内，就删去这个关键词，否则会学到关键词不出现也可以
...
if __name__ == '__main__':
    ...
    parser.add_argument('--raw_data_path', default='data/gaoxiaosong_dict.json', type=str, required=False, help='原始训练语料')
    parser.add_argument('--tokenized_data_path', default='data/tokenized/', type=str, required=False,
                        help='tokenized语料存放位置')
    parser.add_argument('--raw', action='store_true', help='是否先做tokenize')
    parser.add_argument('--epochs', default=300, type=int, required=False, help='训练循环')
